...
class VehicleFactory:
    ...
    @staticmethod
    def create_vehicle(vehicle_type, name, capacity): # Метод для создания объектов классов
    ...
    @staticmethod
    def save_to_json(vehicles, filename): # Метод для сохренения созданных объектов в json
    ...
    def load_from_json(filename): # Метод для выгрузки объектов из json
        if '.json' not in filename:
            ...
            with open(filename, 'r') as file:
                ...
                return [VehicleFactory.create_vehicle(v['type'], v['name'], v['capacity']) for v in vehicles_data]

    @staticmethod
    def save_to_xml(vehicles, filename): # Метод для сохренения созданных объектов в xml
    ...
    def load_from_xml(filename): # Метод для выгрузки объектов из xml
        if '.xml' not in filename:
            ...
            for vehicle_elem in root.findall('Vehicle'):
                ...
                vehicles.append(VehicleFactory.create_vehicle(vehicle_type, name, capacity))
